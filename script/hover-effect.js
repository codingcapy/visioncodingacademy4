...
const mouse = {
    ...
    x(e) {
        return Math.abs(e.clientX - (window.innerWidth / 30));
    },
    y(e) {
        return Math.abs(e.clientY - (window.innerHeight / 30));
    }
};
...
function createShadow(e, currTarget) {
    ...
    const walk = Math.round(Math.max(window.innerWidth, window.innerHeight) / 40); // Or use 150?
    const coordWalk = (coord, side) => Math.round((coord / side * walk) - (walk / 30));
